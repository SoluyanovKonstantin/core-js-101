 ...
 *
 */
function getRectangleString(widthP, heightP) {
  let str = '';
  const width = widthP + 1;
  const height = heightP;
  for (let i = 0; i < height; i += 1) {
    for (let j = 0; j < width; j += 1) {
      if ((String(i) + String(j)) === '00') {
        str += '┌';
      }

      if (((String(i) + String(j))) === `0${width - 2}`) {
        str += '┐';
        j += 1;
        str += '\n';
      }
...
function encodeToRot13(str) {
  ...
  let decodeStr = '';
  for (let i = 0; i < str.length; i += 1) {
    const number = input.indexOf(str[i]);
    decodeStr += output.charAt(number) || ' ';
  }
  return decodeStr;
}
...
function isString(value) {
  return value !== undefined && value !== null && ((value.split && true) || false);
}
