 ...
 *   getPolynom()      => null
 */
function getPolynom(x1 = 0, x2 = 0, x3 = 0) {
  let a = x1;
  let b = x2;
  let c = x3;
...
function logger(func, logFunc) {
  return (...foo) => {
    if (foo.length === 2) {
      logFunc(`${func.name}(["${foo[0][0]}","${foo[0][1]}",${foo[0][2]}],${foo[1]}) starts`);
    } else {
      logFunc(`${func.name}(${foo}) starts`);
    }
    const res = func(...foo);
    if (foo.length === 2) {
      logFunc(`${func.name}(["${foo[0][0]}","${foo[0][1]}",${foo[0][2]}],${foo[1]}) ends`);
    } else {
      logFunc(`${func.name}(${foo}) ends`);
    }
    return res;
  };
}
